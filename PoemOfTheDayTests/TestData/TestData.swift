...
struct TestData {
    ...
    private static func generatePoemForVibe(_ vibe: DailyVibe) -> (title: String, lines: [String]) {
        ...
        default:
            // Handle all new vibes with generic test data
            return (
                title: "\(vibe.displayName) Test Poem",
                lines: [
                    "This is a test poem for \(vibe.displayName.lowercased()) mood,",
                    "Written to verify the code is good,",
                    "With dynamic title generation in place,",
                    "Each vibe now has its unique space."
                ]
            )
        }
    }
