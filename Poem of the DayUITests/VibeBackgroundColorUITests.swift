...
final class VibeBackgroundColorUITests: XCTestCase {
    ...
    func testBackgroundColorAccessibility() throws {
        ...
        if vibeIndicator.exists {
            ...
            let vibeAccessibilityElement = app.otherElements.matching(NSPredicate(format: "label BEGINSWITH 'Current vibe:'")).firstMatch
            XCTAssertTrue(vibeAccessibilityElement.exists, "Vibe information should be accessible to screen readers")
            
            // Verify accessibility label is descriptive
            let accessibilityLabel = vibeAccessibilityElement.label
            XCTAssertTrue(accessibilityLabel.contains("vibe"), "Accessibility label should mention vibe")
        }
    }
